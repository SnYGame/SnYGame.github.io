...
- Doom now triggers when expiring.
...
- [X Wish](/game/status/wish) was added as a Situational \<Triggered Effect: On Removal\> status with the following description: When [Wish](/game/status/wish) expires or breaks, apply X to the affected.
...
- [Monitor](/game/status/monitor) was realigned as a Situational status.
...
- X [Monitor](/game/status/monitor) [Y] was added as a Good \<Triggered Effect: When Specified\> status with the following description: When X is performed, the affected performs Y (without using an action) after X has resolved.
...
- [Stun / Freeze](/game/status/stun) now skips moves and prevents usage of Quick Effects.
...
- Statuses were assigned to classes:
  ...
  - Action Modifiers / Crowd Control Effects: Haste, [Stun / Freeze](/game/status/stun), Sleep, Banish, Encore, Block, Enhancement, Berserk, Spread, Counterattack
...
- [Distance](/game/status/distance) was removed.
