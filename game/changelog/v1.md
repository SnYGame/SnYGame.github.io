...
25 September 2016. These changes were announced on Discord. Note that these changes were not implemented on the document on the this day. The Surrounded changes were implemented on the day before and the Protected changes were implemented on 9 October 2016.

### Statuses

- Surrounded now triggers on all commands that affect another unit.
- Protected now triggers on all commands that came from another unit.
...
- Berserk now causes attacks to randomly target an enemy and is no longer overwritten by Blind.
- Death Sentence now triggers when the status expires.
...
- The Weird status alignment was renamed to Situational status. Statuses in this alignment may be Good or Bad on a case by case basis.
...
- Berserk was realigned as a bad status and no longer increases damage dealt.
- Sword / Enchantment was realigned as a situational status and no longer has duration of the status to inflict as a parameter.
- Immunity, Surrounded, and Bide, were realigned as situational statuses.
- Spell Break was renamed to Spellbreak and was realigned as a situational status.
- Sleep was realigned as a bad status and no longer recovers HP.
...
- Undead / Zombie was realigned as a bad status. Its order was also switched to Zombie / Undead on the document.
...
- A variation of [Distance](/game/status/distance) called One-Way-Distance was added which only delayed effects targeting the affected.
...
- Dizzy was realigned as a bad status.
...
- Sword / Enchantment added an extra parameter for duration of the status inflicted.
- Dicelock [m-n] was added as a weird status with the following description: Restricts the dice’s minimum and maximum value. Does not apply to chances.
- Surrounded was updated to specify that the effect applies prior to the command used.
- The [1.3.0](/game/changelog/v1.html#v1.3.0) changes to Life Link were reverted and the status was renamed to Destiny Bond. Its original name was slightly changed and given to a new weird status called Lifelink with the following description: Any changes in target’s health are also applied to [partner].
- Bide [duration] was added as a good status with the following description: Stores all damage taken. Has effects described by the spell in question.
...
- The [1.3.3](/game/changelog/v1.html#v1.3.3) changes to [Freeze](/game/status/stun) were reverted.
...
5 July 2016. This update features the second status overhaul, introducing status alignments based on whether the status is beneficial or not.

### Statuses

- Status alignments were introduced. They are: Good, Bad, and Weird.
  - The following statuses were given the Good alignment: Attack Up, Defense Up, Invincible, Reflect, Regen, Haste, Vampire, Evasion, Spread, Immunity, Counterattack, Sword / Enchantment, Barrier, and Invisible.
  - The following statuses were given the Bad alignment: Attack Down, Defense Down, Poison / Burn, Guard Break, [Stun / Paralyze](/game/status/stun), [Freeze](/game/status/stun), Death Sentence, Silence, Slow, Banished, Blind, Life Link, and Surrounded.
  - The following statuses were given the Weird alignment: Sleep, Confuse, Berserk, Bait, Undead / Zombie, [Distance](/game/status/distance), and Dizzy.
- Vampire now heals all damage dealt.
- Life Link now only kills the affected if all other linked units die.
- Surrounded now specifies the inflicted status as part of its name.
- Immunity now can specify immunity to an alignment of statuses.
- Enchantment was added as an alias to Sword.
...
- Blind's chance to cause spellcards to not deal damage was increased to 50%.
...
- [Distance](/game/status/distance) [n] was added with the following effect: The target’s actions, as well as actions affecting the target have a delay of n turns (where n is any natural number).
